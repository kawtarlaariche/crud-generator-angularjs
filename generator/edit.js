...
function divCreateModel(model, params) {
    return '<h1 class="page-header">' + model + '</h1><div class="panel panel-default"><div class="panel-heading">Edit ' + model + '</div><div class="panel-body"><form role="form">' + createInputAttribut(model, params) + '<button type="submit" class="btn btn-default" ng-click="update(' + model.toLowerCase() + ')">Edit Button</button></form></div></div>'
}
...
/*function update(model, params) {
    data = []
    for (i = 0; i < params.length; i++) {
        element = model.toLowerCase() + '.' + params[i].attr
        data.push(element)
    }
    return "update(" + data.join(',') + ")"
}*/
